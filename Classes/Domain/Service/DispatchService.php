                    ...
                    foreach ($recipientLists as $recipientList) {
                        ...
                        if($recipients) {
                            foreach ($recipients as $recipient) {
                                ...
                                if($isActive) {
                                    $active = $active + 1;
                                    $recipientData = ['email' => $recipient->getEmail(), 'firstname' => $recipient->getFirstname(), 'lastname' => $recipient->getLastname(), 'gender' => $recipient->getGender(), 'customsalutation' => $recipient->getCustomsalutation(), 'identifier' => $this->persistenceManager->getIdentifierByObject($recipient)];
